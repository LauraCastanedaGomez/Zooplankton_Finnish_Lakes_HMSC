...
if(file.exists(filename)){
  ...
  for(j in 1:nm){
    ...
    if(m$nt>1 & m$nc>1){
      ...
      VP_gamma <- computeVariancePartitioning(m)
      # Guardar matriz de varianza explicada por cada covariable para cada trait
      filename_gamma <- file.path(resultDir, paste0("parameter_estimates_VP_traits_", modelnames[j], ".csv"))
      write.csv(VP_gamma$vals, file = filename_gamma)
